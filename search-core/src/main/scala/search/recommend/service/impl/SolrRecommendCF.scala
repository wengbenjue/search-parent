...
import search.common.clock.{SystemTimerClock, Timing}
...
/**
  ...
  */
class SolrRecommendCF private extends RecommendService with SolrConfiguration {
  ...
  var productQuery: SolrQuery = _

  def init = {
    solrClient = SolrClient(new SolrClientConf())
    cache = KVCache()

    timer = new Timing(new SystemTimerClock(), period, asyncTopN2Cache, "asyncTop2CACHE")
    timer.start()

    productQuery = new SolrQuery()
  }
  ...
  def putToCache(userId: String, number: Int, resultItem: Seq[RecommendResult]) = {
    cache.putR(userId + separator + number, resultItem)
  }

  def getFromCache(userId: String, number: Int): Seq[RecommendResult] = {
    cache.getR(userId + separator + number)
  }
  ...
  override def recommendByUserId(userId: String, number: Int): Seq[RecommendResult] = {
    ...
    //new tmp
    //val resultRecommend = recommendBySolrAndCacheByUserId(userId, number)
...
object SolrRecommendCF {
  ...
  def test2() = {
    ...
    //Thread.sleep(5000)
    println("############" + s.recommendByUserId("15566", 4))
    println("############" + s.recommendByUserId("58338", 4))
    println("############" + s.recommendByUserId("1", 4))
  ...
  def test1() = {
    ...
    solrCf.cache.putR("test123", buffer)
    //Thread.sleep(9000)
