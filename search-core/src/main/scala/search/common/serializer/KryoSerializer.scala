 ...
 * @author soledede
 */
class KryoSerializer(conf: SolrClientConf)
  extends Serializer
  with Logging
  with Serializable {
  ...
  private val classesToRegister = conf.get("crawler.kryo.classesToRegister", "")
    ...
    .map { className =>
      try {
        ...
        case e: Exception =>
          throw new SolrClientException("Failed to load class to register with Kryo", e)
      }
    }
  ...
  def newKryo(): Kryo = {
    ...
    try {
      ...
      case e: Exception =>
        throw new SolrClientException(s"Failed to register classes with Kryo", e)
    } finally {
...
private[search] class KryoSerializerInstance(ks: KryoSerializer) extends SerializerInstance {
