...
import search.solr.client.exception.SolrClientException
 ...
 * @author soledede
 */
private[search] object JettyUtil extends Logging {
 ...
 // (("") /: in)(_ + _))
  ...
  def addFilters(handlers: Seq[ServletContextHandler], conf: SolrClientConf) {
    val filters: Array[String] = conf.get("search.ui.filters", "").split(',').map(_.trim())
    filters.foreach {
      case filter: String =>
        if (!filter.isEmpty) {
          ...
          conf.get("search." + filter + ".params", "").split(',').map(_.trim()).toSet.foreach {
            param: String =>
              if (!param.isEmpty) {
  ...
  def startServiceOnPort[T](
    ...
    // Should never happen
    throw new SolrClientException(s"Failed to start service$serviceString on port $startPort")
  }

  /**
   * Maximum number of retries when binding to a port before giving up.
   */
  def portMaxRetries(conf: SolrClientConf): Int = {
...
private[search] case class ServerInfo(
                                        server: Server,
                                        boundPort: Int,
